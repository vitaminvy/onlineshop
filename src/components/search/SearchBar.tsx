...
export default function SearchBar() {
   ...
   * Process: trim query; if empty do nothing; else navigate to /search?q=
   * Output: route change to SearchPage
   */
  ...
  return (
    <form onSubmit={onSubmit} role="search" className="relative w-full">
      ...
      <button
        ...
        className="absolute right-1 top-1/2 -translate-y-1/2 h-10 rounded-full bg-primary px-5 text-sm font-medium text-white hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed"
      >
        Search
      </button>
    </form>
